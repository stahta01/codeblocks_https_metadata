...
bool SQVM::BW_OP(SQUnsignedInteger op,SQObjectPtr &trg,const SQObjectPtr &o1,const SQObjectPtr &o2)
{
	...
	if((sq_type(o1)==OT_INTEGER) && (sq_type(o2)==OT_INTEGER))
	{
		switch(op) {
...
bool SQVM::ARITH_OP(SQUnsignedInteger op,SQObjectPtr &trg,const SQObjectPtr &o1,const SQObjectPtr &o2)
{
	if(sq_isnumeric(o1) && sq_isnumeric(o2)) {
			...
			if(op == '+' &&	(sq_type(o1) == OT_STRING || sq_type(o2) == OT_STRING)){
...
bool SQVM::ObjCmp(const SQObjectPtr &o1,const SQObjectPtr &o2,SQInteger &result)
{
	if(sq_type(o1)==sq_type(o2)){
		...
		if(sq_isnumeric(o1) && sq_isnumeric(o2)){
			if((sq_type(o1)==OT_INTEGER) && (sq_type(o2)==OT_FLOAT)) {
		...
		else if(sq_type(o1)==OT_NULL) {_RET_SUCCEED(-1);}
		else if(sq_type(o2)==OT_NULL) {_RET_SUCCEED(1);}
...
bool SQVM::StartCall(SQClosure *closure,SQInteger target,SQInteger args,SQInteger stackbase,bool tailcall)
{
	...
	if(sq_type(closure->_env) == OT_WEAKREF) {
	...
	if (sq_type(_debughook) != OT_NULL && _rawval(_debughook) != _rawval(ci->_closure))
		CallDebugHook(_SC('c'));
	return true;
}
...
{
	if(sq_type(o1) != OT_TABLE) { Raise_Error(_SC("delegating a '%s'"), GetTypeName(o1)); return false; }
	switch(sq_type(o2)) {
	case OT_TABLE:
...
bool SQVM::IsFalse(SQObjectPtr &o)
{
	...
	if(( (sq_type(o) & SQOBJECT_CANBEFALSE) && ( (sq_type(o) == OT_FLOAT) && (_float(o) == SQFloat(0.0))) )
...
exception_restore:
	//
	{
		for(;;)
		{
			...
			case _OP_TAILCALL:
				temp_reg = STK(arg1);
				if (sq_type(temp_reg) == OT_CLOSURE && !_funcproto(_closure(temp_reg)->_function)->_bgenerator){
					ct_tailcall = true;
					if(ci->_vargs.size) PopVarArgs(ci->_vargs);
...
common_call:
			...
			case _OP_PREPCALLK:
				{
					...
					STK(arg3) = sq_type(o) == OT_CLASS?STK(0):o;
					TARGET = temp_reg;
				}
				continue;
			case _OP_GETK:
			...
			case _OP_INSTANCEOF:
				if(sq_type(STK(arg1)) != OT_CLASS || sq_type(STK(arg2)) != OT_INSTANCE)
			...
			case _OP_NEWSLOTA:
				...
				if(sq_type(STK(arg1)) == OT_CLASS) {
					if(sq_type(_class(STK(arg1))->_metamethods[MT_NEWMEMBER]) != OT_NULL ) {
		...
		if(ci) {
			...
			if(traps) {
				do {
					...
					if(sq_type(ci->_closure) != OT_CLOSURE && n)
						break;
			...
			if(ci) do {
				...
				PopVarArgs(ci->_vargs);
				POP_CALLINFO(this);
				if( (ci && sq_type(ci->_closure) != OT_CLOSURE) || exitafterthisone) break;
			} while(_callsstacksize);
...
bool SQVM::CallNative(SQNativeClosure *nclosure,SQInteger nargs,SQInteger stackbase,SQObjectPtr &retval,bool &suspend)
{
	...
	if((tcs = nclosure->_typecheck.size())) {
		for(SQInteger i = 0; i < nargs && i < tcs; i++)
			if((nclosure->_typecheck._vals[i] != -1) && !(sq_type(_stack._vals[stackbase+i]) & nclosure->_typecheck[i])) {
                Raise_ParamTypeError(i,nclosure->_typecheck._vals[i],sq_type(_stack._vals[stackbase+i]));
				return false;
			}
	}
	_nnativecalls++;
	...
	if(sq_type(nclosure->_env) == OT_WEAKREF) {
